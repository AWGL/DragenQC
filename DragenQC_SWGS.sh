...
#Use: bash /staging/scripts/DragenQC.sh /data/archive/novaseq/RUN_ID
...
for sampleDir in "$fastqDirTempRun"/Data/*/*; do
    ...
    if [[ $pipelineName =~ "Dragen" ]]; then
       ...
       if [ -d "$resultsDirTempRun"/"$panel"/"$sampleId" ]; then
        ...
        cp "$pipeline_dir"/$pipelineName/"$pipelineName"-"$pipelineVersion"/"$pipelineName".sh  "$resultsDirTempRun"/"$panel"
